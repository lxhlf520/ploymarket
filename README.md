...
| DB 操作瞬时故障 | 6 次退避重试（1s→15s 封顶）；连接类错误先清掉池中半死连接再试（PG 集群代理易掐连），池损坏则重建 | `DB 瞬时故障（ConnectionDoesNotExistError），1.0s 后重试（第 1/6 次）` |
| 领取任务时 DB 抖动 | 退避重试，worker 循环不退出 | `[w2] 领取任务失败（DB 抖动？...），2s 后重试` |
| 任务因 DB 抖动失败 | 撤销本次尝试计数、直接回队（基础设施抖动不烧任务重试机会、不进死信）；DB 彻底不可用时任务留在 running 等租约回收 | `[w2] 事件 xxx 失败（PG 连接抖动，不计次直接回队）` |
